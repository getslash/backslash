...
export default Ember.Mixin.create({
	...
	if ((this.get('is_abandoned') || this.get('is_session_abandoned')) && status === 'running') {
	    return 'abandoned';
	}
	...
	if (this.get('has_any_error') || ['error', 'failure'].indexOf(status) !== -1) {
	    return 'failed';
	}
  ...
  if (status === 'planned') {
    ...
    }.property('status', 'is_abandoned', 'is_session_abandoned', 'num_skipped_tests', 'has_any_error', 'is_running'),
