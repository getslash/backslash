...
def _generate_dockerfile_lines():
    ...
    yield EXPOSE('80')
    yield CMD('service redis-server start && service nginx start && cd /src && python manage.py ensure-secret /persistent/config/000-secret.yml && python manage.py db ensure && python manage.py db upgrade && python manage.py run_uwsgi')
